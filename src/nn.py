...
class NeuralNetwork:
    ...
    Attributes:
        ...
        input_size, hidden_size, output_size: Kerrosten neuronien määrät.
        learning_rate, epochs, batch_size: Säädettävät hyperparametrit.
    ...
    def __init__(self, input_size, hidden_size, output_size, learning_rate, epochs, batch_size):
        """
        Alustaa neuroverkon kerrokset ja hyperparametrit ja kutsuu parametrien alustamisen.

        Args:
            input_size (int): Syötekerroksen neuronien määrä.
            hidden_size (int): Piilokerroksen neuronien määrä.
            output_size (int): Output-kerroksen neuronien määrä.
            learning_rate (float): Oppimisnopeus vaikuttaa siihen, kuinka suuria askelia otetaan
    ...
    def initialize_parameters(self):
        """
        Alustaa neuroverkon parametrit.
        ...
        Returns:
            w1, b1, w2, b2: Painot ja biasit.
        """
        w1 = np.random.randn(self.hidden_size, self.input_size) * 0.1
        b1 = np.random.randn(self.hidden_size, 1) * 0.1
        w2 = np.random.randn(self.output_size, self.hidden_size) * 0.1
        b2 = np.random.randn(self.output_size, 1) * 0.1
        return w1, b1, w2, b2
    ...
    def forward_propagation(self, x):
        ...
        Returns:
            Tuple: Kerrosten aktivoidut outputit (a1, a2) ja summat ennen aktivointia (z1).
        """
    ...
    def backward_propagation(self, x_batch, y_batch, a1, a2, z1):
        """
        Vastavirta-algoritmi minimoi virhettä eli eroa verkon tekemän ennusteen ja todellisten
        luokkien välillä. Tavoitteena on löytää minimointia vastaavat painot ja biasit. Algoritmi
        laskee verkon virheen osittaisderivaatat suhteessa painoihin ja biaseihin. Gradientti-
        menetelmässä osittaisderivaatoista koostuvan gradientin perusteella päivitetään uudet
        parametrien arvot.

        Args:
            x_batch: Array, jonka sarakkeet ovat koulutusdatan sampleja. Pilkottu mini-batcheiksi.
    ...
    def train(self, x_train, y_train, x_test, y_test):
        ...
        jotka viedään verkon läpi eteenpäin suuntautuvassa laskennassa (forward propagation).
        Laskettuja arvoja käytetään taaksepäin suuntautuvassa vastavirta-algoritmissa, joka minimoi
        ennusteen virhettä. Parametrit päivitetään jokaisen mini-batchin ajon jälkeen, eli
        neuroverkko korjaa itseään. Lopuksi kutsutaan arviointifunktiota testidatalla.
    ...
    def save_parameters(self, filename):
        """
        Tallentaa neuroverkon parametrit, konfiguraatiotiedot ja hyperparametrit tiedostoon
        käyttöliittymää varten.

        Args:
            filename: Tiedoston nimi, johon arvot tallennetaan.
        """
        np.savez(filename, w1=self.w1, b1=self.b1, w2=self.w2, b2=self.b2,
                 test_accuracy=self.test_accuracy, input_size=self.input_size,
    ...
    def load_parameters(self, filename):
        """
        Lataa neuroverkon parametrit, konfiguraatiotiedot ja hyperparametrit tiedostoon
        käyttöliittymää varten

        Args:
            filename: Tiedoston nimi, josta arvot ladataan.
        """
...
def preprocess_data(train_file, test_file):
    ...
    Returns:
        Tuple: Esikäsitellyt koulutus- ja testisyötteet ja -arvot.
    """
